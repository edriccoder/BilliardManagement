...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, 'T JAMES SPORTY BAR SALES REPORT (Last 30 Days)', 0, 1, 'C');
...
// SQL query to fetch bookings from the last 30 days, joined with transactions and users tables
$sqlBookings = "
    SELECT 
        b.booking_id, 
    ...
    WHERE 
        DATE(b.start_time) >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND b.status IN ('Confirmed', 'Checked Out')
    ORDER BY 
        b.start_time DESC
";
...
try {
    ...
    if (count($result) > 0) {
        ...
        */
        ...
        // Output the PDF
        $pdf->Output('I', 'Monthly_Sales_Report_Last_30_Days.pdf'); // 'I' for inline display in browser
    } else {
        echo "No bookings found for the last 30 days.";
    }
